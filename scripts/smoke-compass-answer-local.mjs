...
const defaultEndpoint = "http://127.0.0.1:3000/api/compass-answer";
const endpoint =
  process.env.COMPASS_ANSWER_SMOKE_URL ||
  process.env.CHAT_OLLAMA_SMOKE_URL ||
  defaultEndpoint;
const message =
  process.env.COMPASS_ANSWER_SMOKE_QUERY ||
  process.env.CHAT_OLLAMA_SMOKE_QUERY ||
  "광고 심사 기준은 무엇인가요?";

function fail(message, details) {
  console.error(`[smoke:compass-answer] ${message}`);
  if (details) {
...
console.log(JSON.stringify({
  ok: true,
  endpoint: endpoint === defaultEndpoint ? defaultEndpoint : "(configured)",
  schema: process.env.COMPASS_DB_SCHEMA ? "(configured)" : "(default)",
