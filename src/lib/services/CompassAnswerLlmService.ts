...
function buildSystemPrompt(): string {
  return [
    ...
    'If the user question is under-specified, answer the confirmed core first, then ask only the missing follow-up questions needed for execution.',
    'Separate evidence gaps clearly: no verified retrieval for the query, insufficient collected/indexed data, or an out-of-scope detail. Do not fill gaps with memory.',
...
function buildEvidencePrompt(message: string, searchResults: CompassGroundingSource[]): string {
  ...
  return [
    ...
    '- 사용자의 질문이 짧거나 모호해도 검증 근거로 확인되는 핵심부터 답변하고, 실행에 필요한 빠진 조건만 마지막에 "추가 확인 질문"으로 1~3개 제시하세요.',
    '- 근거 부족은 한 문장으로 뭉개지 말고 "검색된 검증 근거 부족", "수집/색인 보강 필요 범위", "질문 범위 밖 세부 조건"처럼 원인을 분리해 설명하세요.',
    ...
    '- 광고 상품/소재 질문은 가능한 경우 "상품명 또는 상품군 / 언제 쓰는지 / 소재·데이터·규격 확인 / 측정·운영 주의점 / 부족한 범위" 순서로 답하세요.',
    '- 네이버·카카오 상품 답변은 검증 근거에 상품명이 있으면 대분류만 쓰지 말고 상품명을 함께 나열하세요. 예시는 검증 근거에 나타난 경우에만 사용하세요: ADVoost 쇼핑, 커뮤니케이션 애드, 치지직, 비즈보드, 상품 카탈로그, 키워드광고, 브랜드검색, 톡채널검색, 보장형/CPT. 근거에 없는 이름은 추가하지 마세요.',
...
function buildCompactRemoteEvidencePrompt(
  ...
  return [
    ...
    '- 질문이 짧거나 모호해도 확인된 핵심부터 답변하고, 실행에 필요한 빠진 조건만 "추가 확인 질문"으로 1~3개 제시하세요.',
    '- 근거 부족은 검색된 검증 근거 부족, 수집/색인 보강 필요 범위, 질문 범위 밖 세부 조건으로 분리해서 말하세요.',
