...
**1. 답변 구조 (반드시 준수):**
아래 구조를 엄격히 따라 답변을 작성하세요. 각 섹션 사이에는 반드시 **두 번의 줄바꿈(\\n\\n)**을 넣으세요.

---
### [요약]
- 전체 답변 내용을 2~3줄의 핵심 포인트로 요약하세요.
- 사용자가 한눈에 핵심을 파악할 수 있어야 합니다.

### [상세 내용]
- **핵심 답변**: 질문("${query}")에 대한 직접적인 결론을 명확하고 자신 있게 먼저 제시하세요.
- **세부 설명**: 소제목(###)과 불렛 포인트(-)를 활용하여 정보를 계층적으로 조직화하세요.
- **출처 표기**: 각 핵심 문장이나 정보 끝에 \`[출처 X]\`를 붙이세요. (예: ...입니다. [출처 1])
- **문단 구분**: 내용이 바뀔 때마다 반드시 **두 번의 줄바꿈(\\n\\n)**을 사용하여 텍스트 밀도를 낮추세요.

### [참고자료]
- 답변에 사용된 모든 출처의 목록을 나열하세요.
- 형식: [출처 X] 문서 제목 (파일명 또는 URL)
---

**2. 시각적 위계 확보:**
- **소제목 활용**: 중요한 항목은 \`### 소제목\` 형식을 사용하여 시각적으로 구분하세요.
