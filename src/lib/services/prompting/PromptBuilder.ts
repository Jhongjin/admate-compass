...
**1. 답변 구조 (반드시 준수):**
아래 구조를 따라 답변을 작성하세요. 각 섹션 사이에는 적절한 줄바꿈을 넣어 구분하세요.
...
### [상세 내용]
- **핵심 답변**: 질문("${query}")에 대한 직접적인 결론을 명확히 먼저 제시하세요.
- **세부 설명**: 정보의 범주가 바뀔 때는 반드시 **소제목(###)**을 작성하고 그 아래에 상세 내용을 불렛 포인트(-)로 조직화하세요.
- **출처 표기**: 각 핵심 문장이나 개별 정보 끝에 \`[출처 X]\`를 붙이세요. (예: ...입니다. [출처 1])
- **문단 구분**: 내용이 바뀔 때 적절히 문단을 나누어 가독성을 확보하세요.

### [참고자료]
- 답변에 사용된 모든 출처의 목록을 나열하세요.
---

**2. 시각적 위계 확보 (매우 중요):**
- **소제목 의무화**: 카테고리나 주요 항목을 나눌 때 단순히 **굵은 글씨**만 사용하지 말고, 반드시 \`### 소제목\` 형식을 사용하여 시각적 위계를 만드세요.
- **가독성 중심**: 리스트가 너무 길어지면 소제목으로 단락을 쪼개어 사용자가 스크롤 부담 없이 읽을 수 있게 하세요.
- **강조 기법**: 핵심 키워드는 **두껍게** 표시하여 강조하세요. (예: **500만원**, **승인 필수**)
